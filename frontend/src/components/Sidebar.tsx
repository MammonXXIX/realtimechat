...
export default function Sidebar({
  ...
  const [tab, setTab] = useState<"contacts" | "chats">("contacts");
      ...
      <div className="flex items-center justify-between px-4 py-4 border-b">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="p-2 rounded-full bg-gray-200 hover:bg-gray-300 shadow-sm text-gray-700 transition"
            title="Create Group"
          >
            <Users size={18} />
          </button>
        ...
        {tab === "contacts" && (
          <>
            {contacts.map((c) => (
              <div
                key={c.roomId}
                className="relative flex items-center justify-between gap-3 p-3 rounded-xl bg-white border shadow-sm hover:shadow-md transition"
              >
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setSelectedMenuContact(
                      selectedMenuContact === c.roomId ? null : c.roomId
                    );
                  }}
                ...
                {selectedMenuContact === c.roomId && (
                  <div className="absolute top-12 right-2 bg-white border shadow-md rounded-lg w-40 z-20">
                    <button
                    ...
                    <button
                      className="w-full text-left px-4 py-2 hover:bg-gray-100 text-red-700"
        ...
        {tab === "chats" &&
          contacts.map((c) => (
            ...
            >
              ...
              {/* Unread badge with number (optional, assuming c.unreadCount exists) */}
              {c.unread && c.messages.length > 0 && (
                <span className="ml-auto inline-flex items-center justify-center px-2 py-1 text-xs font-semibold text-white bg-indigo-600 rounded-full">
                  {c.messages.length}
                </span>
              )}
            </div>
